///////////////////////////// CÓDIGO QUE DEBES UTILIZAR /////////////////////////
const factura = [
    { id: 1, descripcion: "Atún 3 latas 30gr", precio: 2.4, cantidad: 3 },
    { id: 2, descripcion: "Tomate frito 3 briks 390gr", precio: 1.45, cantidad: 1 },
    { id: 3, descripcion: "Café molido 250gr", precio: 3.99, cantidad: 2 },
    { id: 4, descripcion: "Garbanzos cocidos 400gr", precio: 0.85, cantidad: 4 },
    { id: 5, descripcion: "Azúcar blanco 1kg", precio: 1.47, cantidad: 2 },
    { id: 6, descripcion: "Arroz 1kg", precio: 1.35, cantidad: 1 },
    { id: 7, descripcion: "Leche semidesnatada 1l", precio: 1.06, cantidad: 6 },
    { id: 8, descripcion: "Tomate triturado 390gr", precio: 0.72, cantidad: 2 },
    { id: 9, descripcion: "Nata líquida 3 unidades de 200 ml", precio: 1.75, cantidad: 1 },
    { id: 10, descripcion: "Aceite de oliva suave 1l", precio: 4.99, cantidad: 4 }
];
///////////////////////////// FIN CÓDIGO QUE DEBES UTILIZAR /////////////////////////

///////////////////////////// AQUÍ COMIENZA TU CÓDIGO /////////////////////////

// ACTIVIDAD 3:
// Apartado 1:
/**
 * 
 * @param {array} factura 
 * @returns {array}
 * Filtra por precio los productos del array Factura y los ordena de menor a mayor según la cantidad
 */
function obtenerCarosOrdenadosPorCantidad(factura) {
    return factura.filter(producto => producto.precio > 2).sort((producto1, producto2) => producto1.cantidad - producto2.cantidad);
}

const productosOrdenados = obtenerCarosOrdenadosPorCantidad(factura);

productosOrdenados.forEach(producto => {
    console.log(`id: ${producto.id}, descripcion: ${producto.descripcion}, precio: ${producto.precio}, cantidad: ${producto.cantidad}`);
});

console.log("///////////////////////////// SEPARADOR /////////////////////////");
// Apartado 2:
/**
 * 
 * @param {array} factura 
 * @param {int} numElementos 
 * @returns {array}
 * Ordena de mayor a menor por precio el array factura y coge los N productos más caros
 */
function obtenerNMasCaros(factura, numElementos) {
    if(Number.isNaN(numElementos)){
        // No es un número
        console.error("La variable introducida no es un número");
    } else if (numElementos > 0) {
        return factura.sort((p1, p2) => p2.precio - p1.precio).slice(0, numElementos);
    } else {
        return arrayVacio = [];
    }

}

const productosMasCaros = obtenerNMasCaros(factura, 4);

productosMasCaros.forEach(producto => {
    console.log(`id: ${producto.id}, descripcion: ${producto.descripcion}, precio: ${producto.precio}, cantidad: ${producto.cantidad}`);
});

///////////////////////////// AQUÍ TERMINA TU CÓDIGO /////////////////////////